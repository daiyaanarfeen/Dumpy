...
void dot_product(matrix *vec1, matrix *vec2, float *result) {
    ...
    *result = 0;
    matrix *vec3;
    matrix *vec4;
    allocate_matrix(&vec3, vec1->dim.cols, vec1->dim.rows); 
    allocate_matrix(&vec4, vec2->dim.cols, vec2->dim.rows);
    matrix_transpose(vec1, vec3);
    matrix_transpose(vec2, vec4);
    for (int i = 0; i < vec3->dim.cols; ++i) {
        *result += vec3->data[0][i] * vec4->data[0][i];
    }
...
void matrix_multiply(matrix *mat1, matrix *mat2, matrix *dst) {
    ...
    matrix *mat3;
    allocate_matrix(&mat3, mat2->dim.cols, mat2->dim.rows);
    matrix_transpose(mat2, mat3);
    #pragma omp parallel for    
    for (int i = 0; i < mat1->dim.rows; i++)
            /**int num_threads = omp_get_num_threads();
            int thread = omp_get_thread_num();
            int chunk_size = mat3->dim.rows / num_threads;
            j < (thread == (num_threads - 1) ? mat3->dim.rows :
                chunk_size * (thread + 1))
            int j = thread * chunk_size    **/
            for (int j = 0; j < mat3->dim.rows; j++) {
                dst->data[i][j] = 0; //Ensures that the destination matrix is zeros initially. 
                for (int k = 0; k < mat1->dim.cols; k++)
                    dst->data[i][j] += mat1->data[i][k] * mat3->data[j][k];
            }
    free(mat3);
}
